...
SPANK_PLUGIN(load_unload_mqss, 1);
...
int slurm_spank_job_prolog(spank_t sp, int ac, char **av)
{
    ...
    fprintf(fptr_spank_prolog, "Start loading mqss on compute nodes \n");
    fprintf(fptr_spank_prolog, "-------------------------------\n");
    ...
    fprintf(fptr_spank_prolog, " 1. Load rabbitmq: passed \n");


    // load the bash script to load pyqdaemon
    FILE *fp_load_qdaemon;
    char log_qdaemon[1024];
    ...
    fp_load_qdaemon = popen("runuser -u ubuntu -- echo load-qdaemon", "r");
    if (fp_load_qdaemon == NULL){
       fprintf(fptr_spank_prolog, "\t Error: failed to load qdaemon \n");
       exit(1);
    }
    fgets(log_qdaemon, sizeof(log_qdaemon), fp_load_qdaemon);
    pclose(fp_load_qdaemon);
...
int slurm_spank_job_epilog(spank_t sp, int ac, char **av)
{
    ...
    // Write some text to the file
    fprintf(fptr_spank_epilog, "-------------------------------\n");
    fprintf(fptr_spank_epilog, "Unloading mqss on compute nodes\n");
    fprintf(fptr_spank_epilog, "-------------------------------\n");
