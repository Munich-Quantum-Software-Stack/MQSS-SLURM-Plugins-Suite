...
if [[ "$VAR_GRES" == *qpu* ]]
then
  echo "Detected job with QPU requirement: $VAR_GRES" >> $LOG_FILE
  ...
  # CHECK_ENV=$(env)
  # echo "    DEBUG: check env - ${CHECK_ENV}" >> $LOG_FILE
  ...
  # CATPIDFILE=$(cat ${RABBITMQ_PID_FILE})
  # echo "${CATPIDFILE}" >> $LOG_FILE 2>&1
