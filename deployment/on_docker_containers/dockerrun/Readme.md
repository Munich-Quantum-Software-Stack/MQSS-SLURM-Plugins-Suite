...
Scripts to deploy docker containers in `./deploy`. After creating the containers by script, attach them and configure the slurm.

1. Make sure two containers `slurmmaster` and `slurmnode1` can ping each other by hostname
``` Bash
    # Check the config file /etc/hosts
    # and add the addresses of each node
    172.17.0.2      slurmmaster
    172.17.0.3      slurmnode1
```

2. Make sure SSH passwordless
- On master: `$ ssh-keygen && ssh-copy-id admin@slurmnode1`
- On compute node: `$ ssh-keygen && ssh-copy-id admin@slurmmaster`

2. Enable MUNGE - Authentication service for creating and validating user credentials
- On the master node:
``` Bash
...
4. Compile and build slurm unit services from source
```Bash
    # before compiling slurm, need to install libssl-dev
    
    # run configure file to configure slurm before compiling
    cd /home/admin/shared/slurm_storage/slurm/slurm-23.11
    source ./configure_slurm_installed_from_source.sh

    # compile slurm
    make -j 4

    # copy pre-defined unit service files to /etc/systemd/system/multi-user.target.wants
    sudo cp /home/admin/shared/slurm_storage/slurm_configs/*.service /etc/systemd/system/multi-user.target.wants
```
...
- Create some needed folders/files for slurm, can follow the script at `/home/admin/shared/slurm_storage/slurm_configs`, named `script_creat_slurm_folders_files_master.sh`.

- Run the script to enable ports on the master node, `script_enable_ports_master.sh`

- Configure the cgroup under root permission, follow the script `script_cp_configure_cgroup.sh`
